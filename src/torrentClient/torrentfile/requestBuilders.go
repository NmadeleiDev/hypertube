...
func (t *TorrentFile) buildHttpTrackerURL(announce string) (string, error) {
	...
	params := url.Values{
		...
		"peer_id":    []string{string(t.Download.MyPeerId[:])},
		"port":       []string{strconv.Itoa(int(t.Download.MyPeerPort))},
		"uploaded":   []string{"0"},
		"downloaded": []string{"0"},
		"compact":    []string{"1"},
...
func (t *TorrentFile) buildUdpTrackerAnnounceReq() (request []byte, err error) {
	t.Download.TransactionId = uint32(rand.Int31())
	...
	binary.BigEndian.PutUint64(req[0:8], t.Download.ConnectionId)
	...
	copy(req[36:56], t.Download.MyPeerId[:])
	...
	binary.BigEndian.PutUint16(req[96:98], t.Download.MyPeerPort)

	logrus.Infof("my peer id: %v; left: %v; my port: %v", t.Download.MyPeerId, uint64(t.PieceLength * len(t.PieceHashes)), t.Download.MyPeerPort)

	return req, nil
}

func (t *TorrentFile) buildScrapeUdpReq() (request []byte, err error) {
	t.Download.TransactionId = uint32(rand.Int31())

	req := make([]byte, 16 + len(t.InfoHash))
	binary.BigEndian.PutUint64(req[:8], t.Download.ConnectionId)
